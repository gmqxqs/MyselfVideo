package com.shuyu.gsyvideoplayer.danmu;
...
//        }
     ...
     * @param model
     */
    public void addDanmu(final DanmuModel model,int textSize,long textColor) {
        if (adapter == null) {
            throw new Error("DanmuAdapter(an interface need to be implemented) can't be null,you should call setAdapter firstly");
        }
...
//                addTypeView(model, dmView, false);
//        } else {
        dmView = adapter.getView(model, null,textSize,textColor);
        addTypeView(model, dmView, false);
//        }
